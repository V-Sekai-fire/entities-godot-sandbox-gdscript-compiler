...
static double float_immediate(const IRFunction &func, int reg) {
	...
	FAIL("no LOAD_FLOAT_IMM for that register");
	return 0.0;
}
...
static int64_t int_immediate(const IRFunction &func, int reg) {
	...
	FAIL("no LOAD_IMM for that register");
	return 0;
}
...
TEST_CASE("no constructor becomes a call") {
...
TEST_CASE("zero argument forms") {
	...
	REQUIRE(make_v2.operands.size() == 3);
	for (int i = 1; i <= 2; i++) {
		REQUIRE(float_immediate(find_function(v2, "test"),
	...
	REQUIRE(make_color.operands.size() == 5);
	REQUIRE(float_immediate(color_fn, make_color.operands[1].reg_index()) == 0.0);
	REQUIRE(float_immediate(color_fn, make_color.operands[2].reg_index()) == 0.0);
	REQUIRE(float_immediate(color_fn, make_color.operands[3].reg_index()) == 0.0);
	REQUIRE(float_immediate(color_fn, make_color.operands[4].reg_index()) == 1.0);
	...
	REQUIRE(float_immediate(rgb_fn, make_rgb.operands[4].reg_index()) == 1.0);
	...
	REQUIRE(int_immediate(v2i_fn, make_v2i.operands[1].reg_index()) == 0);

	// Typed var without initializer -> same default constructor.
	const IRProgram declared = compile_to_ir(
			"func test():\n\tvar r : Rect2\n\treturn r\n");
	REQUIRE(count_opcode(find_function(declared, "test"), IROpcode::MAKE_RECT2) == 1);
}

// -= Rect2, Rect2i and Plane =-

TEST_CASE("rect and plane") {
	// Dedicated MAKE_RECT2, MAKE_RECT2I, MAKE_PLANE opcodes.
	REQUIRE(count_opcode(find_function(
	...
	REQUIRE(count_opcode(from_vectors_fn, IROpcode::VGET_INLINE) == 4);
	REQUIRE(count_opcode(from_vectors_fn, IROpcode::MAKE_RECT2) == 1);
	...
	REQUIRE(count_opcode(plane_fn, IROpcode::VGET_INLINE) == 3);
	const IRInstruction &make_plane = only(plane_fn, IROpcode::MAKE_PLANE);
	REQUIRE(float_immediate(plane_fn, make_plane.operands[4].reg_index()) == 0.0);
	...
	for (const char *form : host_forms) {
		...
		REQUIRE(count_opcode(fn, IROpcode::MAKE_PLANE) == 0);
		REQUIRE(count_opcode(fn, IROpcode::CONSTRUCT) == 1);
	}
	...
	REQUIRE(count_opcode(plane_copy_fn, IROpcode::MAKE_PLANE) == 1);
	REQUIRE(count_opcode(plane_copy_fn, IROpcode::CONSTRUCT) == 1);

	const IRProgram untyped = compile_to_ir(
			"func test(n, d):\n\treturn Plane(n, d)\n");
	const IRFunction &untyped_fn = find_function(untyped, "test");
	REQUIRE(count_opcode(untyped_fn, IROpcode::MAKE_PLANE) == 0);
	REQUIRE(count_opcode(untyped_fn, IROpcode::CONSTRUCT) == 1);
}
...
TEST_CASE("container constructors") {
	...
	REQUIRE(make_dict.operands.size() == 2);
	REQUIRE(make_dict.operands[1].immediate() == 0);
	...
	REQUIRE(make_packed.operands[1].immediate() == 1);
	...
	REQUIRE(count_opcode(converted.member_init, IROpcode::CONSTRUCT) == 1);
	const IRInstruction &array_construct = only(
			find_function(converted, "array_copy"), IROpcode::CONSTRUCT);
	REQUIRE(array_construct.operands[1].immediate() == Variant::ARRAY);
	const IRInstruction &dictionary_construct = only(
			find_function(converted, "dictionary_copy"), IROpcode::CONSTRUCT);
	REQUIRE(dictionary_construct.operands[1].immediate() == Variant::DICTIONARY);
	REQUIRE(machine_code_builds("func test(value):\n\treturn Array(value)\n"));
}

// -= Arity =-

TEST_CASE("wrong arity is refused") {
	REQUIRE(refuses("func test():\n\treturn Vector2(1)\n"));
...
TEST_CASE("conversions reach the engine") {
	...
	REQUIRE(construct.operands[1].immediate() == Variant::VECTOR2);
	...
	REQUIRE(t2d.operands[1].immediate() == Variant::TRANSFORM2D);
	REQUIRE(t2d.operands[2].immediate() == 2);
...
TEST_CASE("builtin constants") {
	...
	REQUIRE(count_opcode(zero_fn, IROpcode::MAKE_VECTOR2) == 1);
	REQUIRE(count_opcode(zero_fn, IROpcode::VGET) == 0);
	REQUIRE(count_opcode(zero_fn, IROpcode::VCALL) == 0);
	...
	REQUIRE(float_immediate(forward_fn, make_forward.operands[1].reg_index()) == 0.0);
	REQUIRE(float_immediate(forward_fn, make_forward.operands[2].reg_index()) == 0.0);
	REQUIRE(float_immediate(forward_fn, make_forward.operands[3].reg_index()) == -1.0);
	...
	REQUIRE(int_immediate(max_fn, make_max.operands[1].reg_index()) == 2147483647);
	...
	REQUIRE(count_opcode(transform2d_fn, IROpcode::MAKE_VECTOR2) == 3);
	const IRInstruction &make_transform2d = only(transform2d_fn, IROpcode::CONSTRUCT);
	REQUIRE(make_transform2d.operands[1].immediate() == Variant::TRANSFORM2D);
	REQUIRE(make_transform2d.operands[2].immediate() == 3);

	const IRProgram basis = compile_to_ir("func test():\n\treturn Basis.FLIP_Y\n");
	REQUIRE(count_opcode(find_function(basis, "test"), IROpcode::MAKE_VECTOR3) == 3);
	const IRInstruction &make_basis = only(find_function(basis, "test"), IROpcode::CONSTRUCT);
	REQUIRE(make_basis.operands[1].immediate() == Variant::BASIS);
	...
	REQUIRE(make_quaternion.operands[1].immediate() == Variant::QUATERNION);
	REQUIRE(make_quaternion.operands[2].immediate() == 4);

	const IRProgram projection = compile_to_ir("func test():\n\treturn Projection.ZERO\n");
	REQUIRE(count_opcode(find_function(projection, "test"), IROpcode::MAKE_VECTOR4) == 4);
	const IRInstruction &make_projection = only(find_function(projection, "test"), IROpcode::CONSTRUCT);
	REQUIRE(make_projection.operands[1].immediate() == Variant::PROJECTION);

	const IRProgram transform3d = compile_to_ir(
			"func test():\n\treturn Transform3D.FLIP_Z\n");
	REQUIRE(count_opcode(find_function(transform3d, "test"), IROpcode::CONSTRUCT) == 2);
	REQUIRE(machine_code_builds("func test():\n\treturn Transform2D.IDENTITY\n"));
	...
	const IRProgram shadowed = compile_to_ir(
			"func test():\n\tvar Color = {}\n\tColor.RED = 1\n\treturn Color\n");
	REQUIRE(count_opcode(find_function(shadowed, "test"), IROpcode::MAKE_COLOR) == 0);
}

// -= Color8 =-

TEST_CASE("color8 rescales") {
	...
	const IRFunction &test = find_function(ir, "test");
	REQUIRE(count_opcode(test, IROpcode::MAKE_COLOR) == 1);
	REQUIRE(count_opcode(test, IROpcode::DIV) == 3);
	...
	REQUIRE(float_immediate(folded_fn, make.operands[1].reg_index()) == 1.0);
	REQUIRE(float_immediate(folded_fn, make.operands[2].reg_index()) == 0.0);
	REQUIRE(float_immediate(folded_fn, make.operands[4].reg_index()) == 1.0);
