...
TEST_CASE("fast exit stops and jumps back") {
	...
	REQUIRE(read_u32(elf, offset) == STOP_INSN);
	REQUIRE(read_u32(elf, offset + 4) == JUMP_BACK_INSN);
}

TEST_CASE("entry point skips fast exit") {
	...
	REQUIRE(read_u64(elf, 0x18) == BASE_ADDR + FAST_EXIT_SIZE);
}

TEST_CASE("functions start after the entry") {
	const auto elf = build("func first():\n\treturn 1\n\nfunc second():\n\treturn 2\n");
	for (const char *name : { "first", "second" }) {
		const Symbol sym = find_symbol(elf, name);
		REQUIRE(sym.found);
		REQUIRE(sym.value > BASE_ADDR + FAST_EXIT_SIZE);
	}
}
