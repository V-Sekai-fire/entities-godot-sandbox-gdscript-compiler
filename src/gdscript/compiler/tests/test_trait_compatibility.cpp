...
static void parameter(const std::string &required, const std::string &actual, bool compatible) {
	...
	const auto elf = compiler.compile("uses Contract\ntrait Contract:\n\t@abstract func take(value: " + required +
									  ...
									  options());
	if (compatible != !elf.empty())
		std::cerr << required << " <- " << actual << ": " << compiler.get_error() << '\n';
	REQUIRE(compatible == !elf.empty());
	if (!compatible) {
		REQUIRE(compiler.get_error().find("parameter 'value' has incompatible type") != std::string::npos);
		REQUIRE((compiler.get_error_info().line > 0 && compiler.get_error_info().column > 0));
	}
}
...
static void returns(const std::string &required, const std::string &actual, bool compatible) {
	...
	const auto elf = compiler.compile("uses Contract\ntrait Contract:\n\t@abstract func get_value() -> " + required +
									  ...
									  options());
	if (compatible != !elf.empty())
		std::cerr << required << " <- return " << actual << ": " << compiler.get_error() << '\n';
	REQUIRE(compatible == !elf.empty());
	if (!compatible)
		REQUIRE(compiler.get_error().find("returns '") != std::string::npos);
}
...
static ClassSignature api(const std::string &methods) {
	...
	if (elf.empty())
		std::cerr << methods << ": " << compiler.get_error() << '\n';
	REQUIRE(!elf.empty());
	...
	for (const auto &c : decoded)
		if (c.is_trait && c.name == "API")
			return c;
	REQUIRE(false);
	return {};
}

TEST_CASE("offered api") {
	...
	REQUIRE(trait_conformance(expanded.trait_methods, old).empty());
	REQUIRE(!trait_conformance(old.trait_methods, expanded).empty());
	for (const auto &declaration : {
		...
		REQUIRE(!trait_conformance(changed.trait_methods, old).empty());
	}
}

TEST_CASE("trait compatibility is checked structurally") {
