...
TEST_CASE("a method is a lifted function") {
...
TEST_CASE("the instance is a dictionary") {
	const IRProgram ir = compile_to_ir(CHAIN);
...
TEST_CASE("a method call is a direct call") {
	const IRProgram ir = compile_to_ir(CHAIN);
	const IRFunction *test = find_function(ir, "test");
	if (test != nullptr) {
...
TEST_CASE("a field is the instance dictionary") {
...
TEST_CASE("super at script level") {
...
TEST_CASE("self local call is direct and typed") {
	const IRProgram ir = compile_to_ir(
...
TEST_CASE("a class type travels") {
	const IRProgram ir = compile_to_ir(
			"class C:\n"
			"\tvar n = 0\n"
			"\tfunc get_n():\n"
...
TEST_CASE("a lambda in a method sees the class") {
	const IRProgram ir = compile_to_ir(
			"var value = 999\n"
			"class Counter:\n"
			"\tvar value = 7\n"
...
TEST_CASE("a qualified base is refused") {
...
TEST_CASE("a native base is constructed with the instance") {
...
TEST_CASE("the bind syscall is emitted for an engine base") {
...
TEST_CASE("a class typed declaration constructs nothing") {
	const IRProgram ir = compile_to_ir(
			"class Marker extends Node2D:\n"
			"\tvar hits = 0\n"
			"struct Point:\n"
...
TEST_CASE("a class typed member holds null") {
...
TEST_CASE("class signatures are published") {
	const IRProgram ir = compile_to_ir(PUBLISHED);
...
TEST_CASE("super on a native base is marked") {
	const IRProgram ir = compile_to_ir(
			"class Marker extends Node2D:\n"
			"\tfunc get_name():\n"
...
TEST_CASE("what the class does not declare reaches the base") {
	const IRProgram ir = compile_to_ir(NATIVE);
...
TEST_CASE("the script still wins over the base") {
	const IRProgram ir = compile_to_ir(
			"func helper():\n"
			"\treturn 7\n"
...
TEST_CASE("super reaches the native base") {
	const IRProgram ir = compile_to_ir(
			"class C extends Node2D:\n"
			"\tfunc _init():\n"
			"\t\tsuper()\n"
...
TEST_CASE("restrictions refuse what needs a class") {
...
TEST_CASE("a top level extends reaches the owner") {
	const IRProgram ir = compile_to_ir(
			"extends Node2D\n"
			"func test():\n"
...
TEST_CASE("an instance answers is and as") {
...
TEST_CASE("an untyped instance reaches the base") {
	const IRProgram ir = compile_to_ir(
			"class Sprite extends Node2D:\n"
			"\tvar hp = 3\n"
			"func poke(m):\n"
...
TEST_CASE("a class body holds constants and static methods") {
	...
	if (folded_test != nullptr) {
		...
		for (const IRInstruction &instr : folded_test->instructions) {
			if (instr.opcode == IROpcode::LOAD_IMM && instr.operands[1].immediate() == 40) {
				immediates++;
			}
		}
		check(immediates == 2,
...
TEST_CASE("an untracked instance answers is") {
...
TEST_CASE("a class without a base is still refcounted") {
	...
	if (plain_test != nullptr) {
		...
		for (const IRInstruction &instr : plain_test->instructions) {
			if (instr.opcode == IROpcode::LOAD_BOOL && instr.operands[1].immediate() == 1) {
				answered_true = true;
			}
		}
...
TEST_CASE("a method that returns self keeps the type") {
...
std::vector<uint8_t> compile_chain(const std::string &source, const std::vector<Link> &bases) {
	...
	if (elf.empty()) {
		FAIL_CHECK("FAILED to compile chain: ", compiler.get_error());
	}
	return elf;
}
...
TEST_CASE("a chain merges into one program") {
...
TEST_CASE("an override keeps the base copy") {
	const IRProgram ir = compile_chain_to_ir(
			"extends PhysicsUnitTest2D\n"
			"func test_name():\n"
...
TEST_CASE("an overridden setter keeps direct storage access") {
	const IRProgram ir = compile_chain_to_ir(
			"extends Combatant\n"
			"signal resume\n"
...
TEST_CASE("super of the enclosing function") {
...
TEST_CASE("a base static and enum are reachable") {
...
TEST_CASE("what a chain refuses") {
	...
	check(restricted_compiler.compile("extends Base\nfunc f():\n\treturn 1\n",
									  restricted)
				  .empty(),
		  "a restricted Sandbox refuses a base body");
