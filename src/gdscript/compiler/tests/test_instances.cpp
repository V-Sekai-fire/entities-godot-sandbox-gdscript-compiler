...
TEST_CASE("storage classification") {
...
TEST_CASE("initializers are split") {
	const IRProgram ir = compile_to_ir(
...
TEST_CASE("shared initializer cannot read a member") {
...
TEST_CASE("no members no record") {
...
TEST_CASE("member addressing is one instruction") {
...
TEST_CASE("two records do not see each other") {
	Booted booted = boot(
			"static var shared = 0\n"
			"var bump = 0\n"
			"func step():\n"
...
TEST_CASE("a record initializes its members") {
	Booted booted = boot(
			"var a = 7\n"
			"var b = 0\n"
			"func read_a():\n\treturn a\n"
