...
static bool calls_symbol(const IRProgram &ir, const IRFunction &function,
	...
	for (const IRInstruction &instruction : function.instructions) {
		if (instruction.opcode == IROpcode::CALL && !instruction.operands.empty() &&
			...
			ir.strings[instruction.operands[0].string_id] == symbol)
			return true;
	}
	return false;
}
...
TEST_CASE("declaration and operator parse") {
	...
	REQUIRE(program.traits.size() == 1);
	REQUIRE(program.traits[0].name == "Killable");
	REQUIRE(program.traits[0].methods.size() == 2);
	REQUIRE(program.traits[0].methods[1].parameters.size() == 1);
	REQUIRE(program.uses == std::vector<std::string>{ "Killable" });
	REQUIRE(program.structs.size() == 2);
	REQUIRE(program.structs[0].uses == std::vector<std::string>{ "Killable" });
	...
	REQUIRE((test != nullptr && test->type.single_name() == "Killable"));
}

TEST_CASE("trait body restrictions") {
	Program program = parse(
	...
	REQUIRE(trait.base_name == "Node");
	REQUIRE(trait.uses == std::vector<std::string>{ "Base" });
	REQUIRE((trait.vars.size() == 1 && trait.constants.size() == 1));
	REQUIRE((trait.enums.size() == 1 && trait.signals.size() == 1));
	REQUIRE(trait.methods.size() == 3);
	REQUIRE((trait.methods[0].is_static && !trait.methods[0].is_abstract));
	REQUIRE(trait.methods[2].is_abstract);
	REQUIRE(rejection("trait I:\n\t@abstract func run(): pass\n").find("cannot have a body") != std::string::npos);
	REQUIRE(rejection("trait I:\n\tstatic func run()\n").find("must have a body") != std::string::npos);
	REQUIRE(rejection("trait I:\n\tclass Inner:\n\t\tpass\n").find("cannot declare an inner class") != std::string::npos);
	Program file = parse(
			"trait_name Movable\n"
			"extends CharacterBody2D\n"
			"var speed: float = 100.0\n"
			"func move(delta: float) -> void: pass\n");
	REQUIRE(file.trait_name == "Movable");
	REQUIRE((file.traits.size() == 1 && file.traits[0].is_file_level));
	REQUIRE(file.traits[0].base_name == "CharacterBody2D");
}

TEST_CASE("splicing conflicts and displaced calls") {
	...
	REQUIRE((ir.globals.size() == 1 && ir.globals[0].name == "count"));
	REQUIRE(find_function(ir, "@trait.Counter.bump").name == "@trait.Counter.bump");

	const std::string conflict = rejection(
			"trait A:\n\tfunc tick() -> void: pass\n"
			"trait B:\n\tfunc tick() -> void: pass\n"
			"class Broken uses A, B:\n\tpass\n");
	REQUIRE(conflict.find("both declare 'tick'; override it") != std::string::npos);
	compile_to_ir(
			"trait A:\n\tfunc tick() -> void: pass\n"
			"trait B:\n\tfunc tick() -> void: pass\n"
			"class Good uses A, B:\n\tfunc tick() -> void: pass\n");
	...
	links.push_back(ChainLink{ "Base", "base.sgd", parse("class_name Base\nuses Counter\n"
														 "trait Counter:\n\tfunc bump() -> int: return 1\n") });
	links.push_back(ChainLink{ "Derived", "derived.sgd", parse("extends Base\nfunc bump() -> int: return Counter.bump() + 1\n") });
	std::vector<const TraitDecl *> available;
	for (const ChainLink &link : links)
		for (const TraitDecl &trait : link.program.traits)
			available.push_back(&trait);
	for (ChainLink &link : links)
		apply_traits(link.program, available);
	...
	REQUIRE(find_function(chain_ir, "@super0.bump").name == "@super0.bump");
	REQUIRE(calls_symbol(chain_ir, find_function(chain_ir, "bump"), "@super0.bump"));
}

TEST_CASE("base requirements") {
	const std::string source =
			"extends CharacterBody2D\n"
			"uses Movable\n"
			"trait Movable extends Node2D:\n\tfunc move() -> void: pass\n";
	REQUIRE(rejection(source).find("requires base 'Node2D'") != std::string::npos);
	const IRProgram accepted = compile_to_ir(source,
											 { { "CharacterBody2D", "Node2D,CanvasItem,Node,Object" } });
	REQUIRE(accepted.script_uses == std::vector<std::string>{ "Movable" });
	REQUIRE(rejection(
	...
	const IRProgram inherited_requirement = compile_to_ir(
			...
			{ { "Node2D", "CanvasItem,Node,Object" } });
	REQUIRE(inherited_requirement.trait_signatures[1].native_base == "Node");
	const FunctionSignature accept = find_signature(inherited_requirement, "accept");
	REQUIRE(accept.return_type == Variant::OBJECT);
	REQUIRE(accept.return_class_name == "Node");
	REQUIRE(accept.parameters[0].type == Variant::OBJECT);
	REQUIRE(accept.parameters[0].class_name == "Node");
	REQUIRE(rejection(
					"trait Spatial extends Node2D:\n\tpass\n"
...
TEST_CASE("trait constants and enums") {
	const IRProgram ir = compile_to_ir(
			"trait Stateful:\n"
			"\tconst LIMIT := 9\n"
	...
	REQUIRE(count_opcode(find_function(ir, "qualified"), IROpcode::LOAD_IMM) >= 2);
	REQUIRE(count_opcode(find_function(ir, "through_value"), IROpcode::LOAD_IMM) == 1);
	...
	const IRProgram inherited = compile_to_ir(
			...
			"\treturn value.health\n");
	REQUIRE(count_opcode(find_function(inherited, "use"), IROpcode::VCALL) == 1);
	const ClassSignature &derived = inherited.trait_signatures[1];
	REQUIRE(derived.uses == std::vector<std::string>{ "Living" });
	REQUIRE(derived.trait_methods.size() == 1);
	REQUIRE(derived.trait_fields.size() == 1);
	REQUIRE(derived.trait_signals.size() == 1);
}

TEST_CASE("nominal conformance and inheritance") {
	IRProgram ir = compile_to_ir(KILLABLE +
								 "class Base extends Node uses Killable:\n"
								 "\tfunc die() -> void: pass\n"
								 "\tfunc take_damage(amount) -> bool: return true\n\n"
								 "class Child extends Base:\n\tpass\n");
	REQUIRE(ir.trait_signatures.size() == 1);
	REQUIRE(ir.class_signatures.size() == 2);
	const ClassSignature &child = ir.class_signatures.back();
	REQUIRE(child.name == "Child");
	REQUIRE(child.uses == std::vector<std::string>{ "Killable" });

	const std::string missing = rejection(KILLABLE +
										  "class Broken uses Killable:\n"
										  "\tfunc die() -> void: pass\n");
	REQUIRE(missing.find("does not implement abstract method 'take_damage()'") != std::string::npos);

	const std::string arity = rejection(
			"trait I:\n\tfunc run(value: int) -> void\n\n"
			"class Broken uses I:\n\tfunc run() -> void: pass\n");
	REQUIRE(arity.find("takes 0 parameters") != std::string::npos);

	const std::string return_type = rejection(
			"trait I:\n\tfunc run() -> int\n\n"
			"class Broken uses I:\n\tfunc run(): return 1\n");
	REQUIRE(return_type.find("untyped return") != std::string::npos);
}

TEST_CASE("folded and dynamic tests") {
	const IRProgram ir = compile_to_ir(KILLABLE +
									   ...
									   "func dynamic(value):\n\treturn value is Killable\n");
	REQUIRE(count_opcode(find_function(ir, "folded"), IROpcode::TRAIT_TEST) == 0);
	REQUIRE(count_opcode(find_function(ir, "builtin"), IROpcode::TRAIT_TEST) == 0);
	REQUIRE(count_opcode(find_function(ir, "dynamic"), IROpcode::TRAIT_TEST) == 1);
}

TEST_CASE("narrowing and trait calls") {
	const IRProgram ir = compile_to_ir(KILLABLE +
									   "func narrowed(value):\n"
									   ...
									   "\tif value != null:\n"
									   "\t\tvalue.die()\n");
	REQUIRE(count_opcode(find_function(ir, "narrowed"), IROpcode::TRAIT_TEST) == 1);
	REQUIRE(count_opcode(find_function(ir, "narrowed"), IROpcode::VCALL) == 1);
	REQUIRE(count_opcode(find_function(ir, "typed"), IROpcode::VCALL) == 1);
	REQUIRE(count_opcode(find_function(ir, "nullable"), IROpcode::VCALL) == 1);

	const std::string unknown = rejection(KILLABLE +
										  "func broken(value: Killable):\n\tvalue.heal()\n");
	REQUIRE(unknown.find("'Killable' has no method 'heal'") != std::string::npos);
}

TEST_CASE("trait typed containers") {
	const IRProgram ir = compile_to_ir(KILLABLE +
									   ...
									   "\tvalues.append(value)\n");
	REQUIRE(count_opcode(find_function(ir, "visit"), IROpcode::TRAIT_TEST) == 1);
	REQUIRE(count_opcode(find_function(ir, "visit"), IROpcode::VCALL) == 1);
	REQUIRE(count_opcode(find_function(ir, "first"), IROpcode::TRAIT_TEST) == 1);
	REQUIRE(count_opcode(find_function(ir, "add"), IROpcode::TRAIT_TEST) == 1);
}

TEST_CASE("signature publication and backend cache") {
	const IRProgram ir = compile_to_ir(
			"class_name Enemy\n"
			"extends Node\n"
			"uses Stateful\n"
	...
	REQUIRE(ir.trait_signatures.size() == 1);
	REQUIRE(ir.trait_signatures[0].is_trait);
	REQUIRE(ir.trait_signatures[0].trait_methods.size() == 2);
	REQUIRE(ir.trait_signatures[0].trait_fields.size() == 1);
	REQUIRE(ir.trait_signatures[0].trait_signals.size() == 1);
	...
	REQUIRE(decoded[0].is_trait);
	REQUIRE(decoded[0].trait_methods[1].name == "take_damage");
	REQUIRE(decoded[0].trait_fields[0].name == "health");
	REQUIRE(decoded[0].trait_signals[0].name == "changed");
	...
	REQUIRE(backend.get_global_data_size() >= 64 * 16);
}
...
static uint64_t elf_symbol(const std::vector<uint8_t> &elf, const std::string &name,
	...
	for (uint64_t i = 0; i < section_count; i++) {
		...
		for (size_t at = symbols; at + 24 <= symbols + symbols_size; at += 24) {
			if (name != reinterpret_cast<const char *>(&elf[strings + u32(at)]))
				continue;
			if (r_size != nullptr)
				*r_size = u64(at + 16);
			return u64(at + 8);
...
TEST_CASE("trait caches are exported symbols") {
	const IRProgram ir = compile_to_ir(KILLABLE +
									   "trait Movable:\n\tfunc step() -> void\n\n"
									   "func first(value):\n\treturn value is Killable\n\n"
									   "func second(value):\n\treturn value is Movable\n");
	REQUIRE(ir.trait_signatures.size() == 2);
	...
	REQUIRE(first != 0);
	REQUIRE(size == TraitCacheLayout::AREA_SIZE);
	const uint64_t second = elf_symbol(elf, prefix + "1");
	REQUIRE(second == first + TraitCacheLayout::AREA_SIZE);
	REQUIRE(elf_symbol(elf, prefix + "2") == 0);
}

TEST_CASE("an unassigned trait local is not proven") {
	const IRProgram ir = compile_to_ir(KILLABLE +
									   "struct Crate uses Killable:\n"
									   "\tfunc die() -> void: pass\n"
									   "\tfunc take_damage(amount: int = 1) -> bool: return true\n\n"
	...
	REQUIRE(count_opcode(find_function(ir, "declared"), IROpcode::TRAIT_TEST) == 1);
	REQUIRE(count_opcode(find_function(ir, "assigned"), IROpcode::TRAIT_TEST) == 0);

	const std::string unproven = rejection(KILLABLE +
										   "func broken():\n\tvar d: Killable\n\td.die()\n");
	REQUIRE(unproven.find("may be null") != std::string::npos);
}

TEST_CASE("a nullable trait needs a null check") {
	const std::string unchecked = rejection(KILLABLE +
											"func broken(value: Killable?):\n\tvalue.die()\n");
	REQUIRE(unchecked.find("may be null") != std::string::npos);
	...
	const std::string bound = rejection(KILLABLE +
										...
										"\t\tvar bound:\n"
										"\t\t\tbound.die()\n");
	REQUIRE(bound.find("may be null") != std::string::npos);
	...
	REQUIRE(count_opcode(find_function(guarded, "safe"), IROpcode::VCALL) == 1);
}
...
TEST_CASE("file level trait sources") {
	...
	REQUIRE(compiler.get_script_uses() == std::vector<std::string>{ "Movable" });
	REQUIRE(find_published_class(compiler, "Movable").is_trait);
}
...
TEST_CASE("transitive file level trait sources") {
	...
	REQUIRE(!elf.empty());
	REQUIRE(compiler.get_error().empty());
	REQUIRE(compiler.get_script_uses() == std::vector<std::string>({ "Movable", "Powered" }));
	REQUIRE(find_published_class(compiler, "Movable").is_trait);
	REQUIRE(find_published_class(compiler, "Powered").is_trait);
}

TEST_CASE("two qualified traits from one provider") {
	...
	REQUIRE(!elf.empty());
	REQUIRE(compiler.get_error().empty());
	REQUIRE(find_published_class(compiler, "TraitLibrary.Alpha").is_trait);
	REQUIRE(find_published_class(compiler, "TraitLibrary.Beta").is_trait);
	REQUIRE(find_published_class(compiler, "Unused").is_trait);
}
