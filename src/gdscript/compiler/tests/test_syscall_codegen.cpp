...
TEST_CASE("the compiler emits counted syscalls the ABI accepts") {
	...
	const Signature expected[] = {
		{ ECALL_ARRAY_SIZE, -1, 1, 1 },
		{ ECALL_STRING_SIZE, -1, 1, 1 },
		{ ECALL_STRING_AT, -1, 2, 1 },
		{ ECALL_STRING_BATCH, -1, 3, 1 },
		{ ECALL_ARRAY_BATCH, -1, 4, 1 },
		{ ECALL_STRING_CODEPOINT_BATCH, -1, 4, 1 },
		{ ECALL_CALLABLE_CREATE, -1, 4, 1 },
		{ ECALL_VCREATE, -1, 4, 0 },
		{ ECALL_ARRAY_AT, -1, 3, 1 },
	...
	if (const char *fixtures = std::getenv("GDSC_SYSCALL_FIXTURES")) {
		std::filesystem::path out(fixtures);
