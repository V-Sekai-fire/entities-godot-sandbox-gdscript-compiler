...
TEST_CASE("the table is consistent") {
	...
	for (const char *name : NAMES) {
		...
		REQUIRE(std::string(info->name) == name);
		REQUIRE(info->min_args <= info->max_args);

		// The row a GlobalFn resolves to has to be the row it came from.
		REQUIRE(&global_function(info->fn) == info);

		if (info->kind == GlobalKind::SYSCALL) {
			REQUIRE((info->utility_op >= 0 && info->utility_op < UTILITY_OP_COUNT));
			REQUIRE(info->float_args <= UTILITY_MAX_FLOAT_ARGS);
			// A syscall form takes exactly as many arguments as it passes.
			REQUIRE((info->min_args == info->float_args && info->max_args == info->float_args));
		}
		if (info->kind == GlobalKind::SYSCALL_INT) {
			REQUIRE((info->utility_op >= 0 && info->utility_op < UTILITY_OP_COUNT));
			// a1-a3 is all there is to pass integers in.
			REQUIRE(info->min_args == info->max_args);
			REQUIRE(info->max_args <= UTILITY_MAX_INT_ARGS);
			// The answer comes back in a0, as an integer or a boolean.
			REQUIRE((info->result == GlobalResult::INT || info->result == GlobalResult::BOOL));
		}
		if (info->kind == GlobalKind::PRINT) {
			// Each output global must have a valid, unique channel.
			REQUIRE((info->utility_op >= 0 && info->utility_op < int16_t(Print_Channel::CHANNEL_COUNT)));
			REQUIRE(info->result == GlobalResult::NIL);
		}
		if (info->kind == GlobalKind::CAST) {
			...
			REQUIRE((info->min_args == 1 && info->max_args == 1));
			REQUIRE((info->utility_op >= 0 && info->utility_op < UTILITY_OP_COUNT));
			const GlobalFunction &inline_form = global_function(info->int_form);
			REQUIRE(inline_form.result == info->result);
			REQUIRE(resolve_cast_form(*info, Variant::INT) == info->int_form);
			REQUIRE(resolve_cast_form(*info, Variant::FLOAT) == info->int_form);
			REQUIRE(resolve_cast_form(*info, Variant::BOOL) == info->int_form);
			// A String is the reason the host has to be asked at all.
			REQUIRE(resolve_cast_form(*info, Variant::STRING) == info->fn);
			REQUIRE(resolve_cast_form(*info, IRInstruction::TypeHint_NONE) == info->fn);
		}
		...
		const bool is_random = (std::string(info->name).rfind("rand", 0) == 0 &&
				...
				std::string(info->name) == "seed";
		REQUIRE(info->impure == is_random);
		if (info->kind == GlobalKind::NUMERIC) {
			REQUIRE(info->result == GlobalResult::NUMERIC);
			const GlobalFunction &as_int = global_function(info->int_form);
			const GlobalFunction &as_float = global_function(info->float_form);
			REQUIRE(as_int.result == GlobalResult::INT);
			REQUIRE((as_float.result == GlobalResult::INT || as_float.result == GlobalResult::FLOAT));
			REQUIRE(resolve_numeric_form(*info, true) == info->int_form);
			REQUIRE(resolve_numeric_form(*info, false) == info->float_form);
		}
	}
	...
	REQUIRE(find_global_function("no_such_global") == nullptr);
	REQUIRE(find_global_function("randomize") != nullptr);
	REQUIRE(find_global_function("seed") != nullptr);
	REQUIRE(find_global_function("randomize")->unrestricted_only);
	REQUIRE(find_global_function("seed")->unrestricted_only);
	REQUIRE(find_global_function(".int_identity") != nullptr);
	REQUIRE(find_global_function(".float_identity") != nullptr);
	REQUIRE(find_global_function(".booleanize") != nullptr);
}

// -= Lowering =-

TEST_CASE("print is still print") {
	...
	REQUIRE(count_opcode(func, IROpcode::PRINT) == 1);
	REQUIRE(count_opcode(func, IROpcode::GLOBAL_CALL) == 0);

	// Nothing about a global becomes a VCALL on the owner node, which is what
	// the self-call fallback would have produced.
	REQUIRE(count_opcode(func, IROpcode::VCALL) == 0);
}

TEST_CASE("the output channels are one opcode") {
	...
	for (const auto &one : CASES) {
		...
		for (const auto &instr : func.instructions) {
			if (instr.opcode == IROpcode::PRINT) {
				REQUIRE(instr.operands[1].immediate() == int64_t(one.channel));
			}
		}
	}

	// push_error() and push_warning() require an argument.
	REQUIRE(!compile_error("func test():\n\tpush_error()\n").empty());
	REQUIRE(!compile_error("func test():\n\tpush_warning()\n").empty());
}

TEST_CASE("every other global is one opcode") {
	IRProgram ir = compile_to_ir(
			"func test():\n"
			"\treturn absi(-1) + roundi(1.5) + len(\"ab\")\n");
	const IRFunction &func = find_function(ir, "test");
	REQUIRE(count_opcode(func, IROpcode::GLOBAL_CALL) == 3);
	REQUIRE(count_opcode(func, IROpcode::VCALL) == 0);

	const std::vector<GlobalFn> called = called_globals(func);
	REQUIRE(called.size() == 3);
	REQUIRE(called[0] == GlobalFn::ABSI);
	REQUIRE(called[1] == GlobalFn::ROUNDI);
	REQUIRE(called[2] == GlobalFn::LEN);
}

TEST_CASE("numeric dispatch resolves when the types are known") {
	...
	const IRFunction &func = find_function(ir, "test");
	const std::vector<GlobalFn> called = called_globals(func);
	REQUIRE(called.size() == 4);
	REQUIRE(called[0] == GlobalFn::ABSI);
	REQUIRE(called[1] == GlobalFn::ABSF);
	REQUIRE(called[2] == GlobalFn::MINI);
	REQUIRE(called[3] == GlobalFn::MINF);
	REQUIRE(all_calls_typed(func));
}

TEST_CASE("numeric dispatch survives to the backend") {
	...
	REQUIRE(called[0] == GlobalFn::ABS);
	REQUIRE(!all_calls_typed(func));
}

TEST_CASE("int arguments widen for float forms") {
	...
	IRProgram ir = compile_to_ir(
			...
			"\treturn sqrt(16)\n");
	const IRFunction &func = find_function(ir, "test");
	REQUIRE(count_opcode(func, IROpcode::CONVERT) == 1);
	REQUIRE(all_calls_typed(func));
}

TEST_CASE("variadic min and max fold") {
	...
	for (GlobalFn fn : called_globals(func)) {
		REQUIRE(fn == GlobalFn::MINI);
	}
	REQUIRE(run_int("func test():\n\treturn min(4, 3, 2, 1)\n") == 1);
	REQUIRE(run_int("func test():\n\treturn max(4, 3, 2, 1)\n") == 4);
}

TEST_CASE("a global result is typed") {
	...
	for (const auto &instr : find_function(floats, "test").instructions) {
		if (instr.opcode == IROpcode::GLOBAL_CALL) {
			REQUIRE(instr.type_hint == Variant::FLOAT);
		}
	}
...
TEST_CASE("only print and the random draws have side effects") {
	...
	REQUIRE(ir_is_pure(IROpcode::GLOBAL_CALL));
	REQUIRE(!ir_is_pure(IROpcode::PRINT));
	...
	REQUIRE(ir_instruction_is_pure(global_call_instr(GlobalFn::ABSI)));
	REQUIRE(!ir_instruction_is_pure(global_call_instr(GlobalFn::RANDI)));
	REQUIRE(!ir_instruction_is_pure(global_call_instr(GlobalFn::RANDF)));
	...
	IRProgram printed = compile_to_ir(
			...
			"\treturn 1\n",
			true);
	REQUIRE(count_opcode(find_function(printed, "test"), IROpcode::PRINT) == 1);
}

TEST_CASE("random calls survive the optimizer") {
	...
	REQUIRE(count_opcode(optimized, IROpcode::GLOBAL_CALL) == 1);
	...
	REQUIRE(dead_loads == 0);
}

// -= The type constructors =-

TEST_CASE("type constructors lower inline when the type is known") {
	...
	REQUIRE(called.size() == 1);
	REQUIRE(called[0] == GlobalFn::BOOLEANIZE);
	REQUIRE(count_opcode(find_function(ir, "test"), IROpcode::CONVERT) >= 2);
	...
	REQUIRE(host.size() == 3);
	REQUIRE(host[0] == GlobalFn::TO_INT);
	REQUIRE(host[1] == GlobalFn::TO_FLOAT);
	REQUIRE(host[2] == GlobalFn::TO_BOOL);
	...
	REQUIRE(as_string.size() == 1);
	REQUIRE(as_string[0] == GlobalFn::TO_STRING);
	...
	REQUIRE(count_opcode(find_function(ir, "test"), IROpcode::VCALL) == 0);
	REQUIRE(count_opcode(find_function(untyped, "convert"), IROpcode::VCALL) == 0);
}

TEST_CASE("constants fold through casts and inline globals") {
	IRProgram ir = compile_to_ir(
			"func test():\n"
	...
	const IRFunction func = find_function(ir, "test");
	REQUIRE(count_opcode(func, IROpcode::GLOBAL_CALL) == 0);
	REQUIRE(count_opcode(func, IROpcode::CONVERT) == 0);
	REQUIRE(count_opcode(func, IROpcode::ADD) == 0);
	bool found_nine = false;
	for (const IRInstruction &instr : func.instructions) {
		found_nine = found_nine || (instr.opcode == IROpcode::LOAD_IMM && instr.operands[1].immediate() == 9);
	}
	REQUIRE(found_nine);
}

TEST_CASE("type constructors compute") {
	REQUIRE(run_int("func test():\n\tvar f: float = 2.9\n\treturn int(f)\n") == 2);
	REQUIRE(run_int("func test():\n\tvar f: float = -2.9\n\treturn int(f)\n") == -2);
	REQUIRE(run_int("func test():\n\tvar i: int = 7\n\treturn int(i)\n") == 7);
	REQUIRE(close_enough(run_float("func test():\n\tvar i: int = 7\n\treturn float(i)\n"), 7.0));
	REQUIRE(close_enough(run_float("func test():\n\tvar f: float = 2.5\n\treturn float(f)\n"), 2.5));
	REQUIRE(run_bool("func test():\n\tvar i: int = 7\n\treturn bool(i)\n") == true);
	REQUIRE(run_bool("func test():\n\tvar i: int = 0\n\treturn bool(i)\n") == false);
	REQUIRE(run_bool("func test():\n\tvar f: float = 0.5\n\treturn bool(f)\n") == true);
	REQUIRE(run_bool("func test():\n\tvar f: float = 0.0\n\treturn bool(f)\n") == false);
...
TEST_CASE("random is not evaluated by the interpreter") {
	...
	for (const char *source : {
		...
		try {
			...
			threw = true;
			REQUIRE(mentions(e.what(), "random"));
		}
		REQUIRE(threw);
	}

	// The arity check still names them.
	REQUIRE(mentions(compile_error("func test():\n\treturn randi(1)\n"),
...
TEST_CASE("arity is checked by name") {
	REQUIRE(mentions(compile_error("func test():\n\treturn abs()\n"),
...
TEST_CASE("a local function wins") {
	...
	const IRFunction &func = find_function(ir, "test");
	REQUIRE(count_opcode(func, IROpcode::GLOBAL_CALL) == 0);
	REQUIRE(count_opcode(func, IROpcode::CALL) == 1);
	REQUIRE(run_int(
					"func abs(x):\n"
					"\treturn x + 100\n"
					"func test():\n"
...
TEST_CASE("host globals are rejected by the interpreter") {
...
TEST_CASE("numeric forms keep the type") {
	// The whole point of the dispatcher: an integer in, an integer out.
	REQUIRE(std::holds_alternative<int64_t>(run("func test():\n\treturn abs(-2)\n")));
	REQUIRE(std::holds_alternative<double>(run("func test():\n\treturn abs(-2.0)\n")));
	REQUIRE(run_int("func test():\n\treturn floor(-2)\n") == -2);
	REQUIRE(close_enough(run_float("func test():\n\treturn floor(-2.5)\n"), -3.0));
	REQUIRE(close_enough(run_float("func test():\n\treturn ceil(-2.5)\n"), -2.0));
	REQUIRE(close_enough(run_float("func test():\n\treturn round(-2.5)\n"), -3.0));
	...
	const char *dispatched =
			...
			"func test():\n"
			"\treturn measure(-3)\n";
	REQUIRE(run_int(dispatched) == 3);
	const char *dispatched_float =
			"func measure(x):\n"
			"\treturn abs(x)\n"
			"func test():\n"
			"\treturn measure(-3.5)\n";
	REQUIRE(close_enough(run_float(dispatched_float), 3.5));
}

TEST_CASE("float forms") {
	REQUIRE(close_enough(run_float("func test():\n\treturn absf(-2.5)\n"), 2.5));
	REQUIRE(close_enough(run_float("func test():\n\treturn sqrt(9.0)\n"), 3.0));
	REQUIRE(close_enough(run_float("func test():\n\treturn minf(1.5, 2.5)\n"), 1.5));
	REQUIRE(close_enough(run_float("func test():\n\treturn maxf(1.5, 2.5)\n"), 2.5));
...
TEST_CASE("optimizing does not change the answer") {
	...
	for (const char *source : PROGRAMS) {
		...
		REQUIRE(plain.index() == optimized.index());
		if (std::holds_alternative<int64_t>(plain)) {
			REQUIRE(std::get<int64_t>(plain) == std::get<int64_t>(optimized));
		} else {
			REQUIRE(close_enough(std::get<double>(plain), std::get<double>(optimized)));
		}
	}
}
...
TEST_CASE("every global reaches riscv") {
...
TEST_CASE("a global enum member is an immediate") {
...
TEST_CASE("a global enum agrees with the bare constant") {
	...
	for (size_t i = 0; i < global_enum_value_count(); i++) {
		...
		if (bare == nullptr) {
			REQUIRE(std::string(global_enum_value_enum(i)).rfind("Variant.", 0) == 0);
			continue;
		}
		REQUIRE(!bare->is_float);
		REQUIRE(bare->int_value == global_enum_value(i));
		checked++;
	}
...
TEST_CASE("a declared enum shadows the global one") {
	REQUIRE(run_int(
...
TEST_CASE("a global enum is an int type") {
...
TEST_CASE("a global enum is not a value") {
	...
	REQUIRE(mentions(compile_error(
							 "func test():\n"
							 "\treturn Side\n"),
...
TEST_CASE("a local name wins over a global enum") {
	REQUIRE(run_int(
					"func test():\n"
					"\tvar Side = 7\n"
					"\treturn Side\n") == 7);
...
TEST_CASE("is instance of has static and dynamic paths") {
	const GlobalFunction *row = find_global_function("is_instance_of");
	REQUIRE(row != nullptr);
	REQUIRE(row->kind == GlobalKind::HOST);
	REQUIRE((row->min_args == 2 && row->max_args == 2));
	REQUIRE(row->result == GlobalResult::BOOL);
	REQUIRE(row->utility_op == UTILITY_IS_INSTANCE_OF);
	...
	REQUIRE(count_opcode(find_function(native, "test"), IROpcode::GLOBAL_CALL) == 0);
	...
	REQUIRE((called.size() == 1 && called[0] == GlobalFn::IS_INSTANCE_OF));
	...
	REQUIRE(count_opcode(find_function(structure, "test"), IROpcode::GLOBAL_CALL) == 0);

	std::cout << "  ✓ is_instance_of chooses the static type test or host descriptor path"
			  << std::endl;
}

TEST_CASE("inherited engine enum names resolve to the declaring class") {
	REQUIRE(engine_class_declares_enum("Object", "ConnectFlags"));
	REQUIRE(engine_class_declares_enum("Window", "Mode"));
	REQUIRE(engine_class_declares_enum("Node", "ProcessMode"));
	REQUIRE(!engine_class_declares_enum("Node", "ConnectFlags"));
	...
	REQUIRE((has_object && has_constant));
