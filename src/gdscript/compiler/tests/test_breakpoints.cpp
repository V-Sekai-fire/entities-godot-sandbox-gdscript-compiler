...
void check_lines(const std::vector<uint32_t> &actual, const std::vector<uint32_t> &expected,
	...
	FAIL_CHECK("FAILED: ", what, ": expected ", join(expected), ", got ", join(actual));
}
...
void breakpoint_syscall(machine_t &machine) {
	...
	stop.pc_line = g_lines != nullptr
			? g_lines->line_for_address(uint32_t(machine.cpu.pc()))
			: 0;
...
TEST_CASE("nothing emitted without breakpoints") {
...
TEST_CASE("stops on the line it was given") {
...
TEST_CASE("a line with no code is not a stop") {
...
TEST_CASE("a folded line is not a stop") {
...
TEST_CASE("every breakpoint line gets a site") {
...
TEST_CASE("a loop body stops every pass") {
...
TEST_CASE("a for header owns code in two places") {
...
TEST_CASE("a break is transparent to the allocator") {
	...
	for (uint32_t line = 4; line <= 12; line++) {
		...
		check_eq(broken, plain, "a break on line " + std::to_string(line) + " leaves the answer alone");
...
TEST_CASE("registers survive the break") {
...
TEST_CASE("a break carries a call stack") {
...
TEST_CASE("the breakpoint statement") {
	const std::string source =
			"func test(n):\n" // 1
			"\tvar total = 0\n" // 2
			"\tif n > 0:\n" // 3
...
TEST_CASE("breakpoints in more than one function") {
	const std::string source =
			"func left():\n" // 1
			"\treturn 2\n" // 2
			"func right():\n" // 3
...
TEST_CASE("a typed parameter reads back after assignment") {
